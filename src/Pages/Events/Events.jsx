...
export default function Events() {
    ...
    const notify = () => toast.success('Purchased Successfully', {
        position: "top-right",
        autoClose: 5000,
            ...
            <div className="grid md:grid-cols-3 gap-6">
                {eventsData.map(event => (
                    <div key={event.id} className="bg-white shadow-md rounded-lg overflow-hidden hover:shadow-lg transition">
                        <div className="p-5">
                            <span className={`inline-block px-3 py-1 rounded-full text-xs font-semibold mb-3 ${event.type === "Online" ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"
                                }`}>
                                {event.type}
                            </span>
